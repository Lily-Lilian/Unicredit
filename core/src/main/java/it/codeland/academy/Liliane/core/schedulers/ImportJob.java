package it.codeland.academy.Liliane.core.schedulers;
...
public class ImportJob implements Runnable {
    ...
    private final int FIELDS_MAX = 6;
    ...
    private final int FIELD_INDEX_DATE = 5;
    ...
    private void addScheduler(Config config) {
        logger.debug("Adding import job scheduler");
    ...
    protected void deactivate(Config config){
        logger.debug("Stopping import job");
        removeScheduler();
    }

    private void removeScheduler() {
        logger.debug("Removing import job scheduler");
    ...
    @Override
    public void run() {
        String pagePath = "/content/ucs-exercise-liliane/magazine";
        String templatePath = "/apps/ucs-exercise-liliane/templates/articlepage";
        Page articlePage;
        ...
        try {
            ...
            Page articles = pageManager.getPage("/content/ucs-exercise-liliane/magazine/");
            if(articles == null){
            ...
            while (rows.hasNext()) {
                final String[] row = rows.next();
                    ...
                    logger.info("This is our ROW: {}", row);
                    ...
                    if(pageManager.getPage("/content/ucs-exercise-liliane/magazine/" + articleData.pageName + "/") != null){
                        statistics.skippedRecords++;
                        logger.info("Page \"{}\" already exists", articleData.pageName);
                        continue;
                    }
                    articleData.title = row[FIELD_INDEX_PAGE_TITLE];
                    ...
                    articleData.date =  row[FIELD_INDEX_DATE];
                    DateTime dt = new DateTime(articleData.date);
                    ...
                    if (articlePage != null) {
                        ...
                        if (articleContentNode != null) {
                            articleContentNode.setProperty("jcr:title", articleData.title);
                    ...
                    logger.error("Processing failed on row: {} {}", Arrays.asList(row), row.length);
                }
            }
            session.save();
            resolver.close();
