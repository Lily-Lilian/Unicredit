...
import it.codeland.academy.Liliane.core.servlets.ArticleBean;
            ...
            for (int i=0; i<values.length; i++){
                Resource articleResource= resolver.getResource(values[i].getString());
                 Node articleNode = articleResource.adaptTo(Node.class).getNode("jcr:content");
                   ...
                   String dateString = new SimpleDateFormat("E dd MMM yyyy").format(date);
                   ArticleBean article = new ArticleBean(title,tags,image,title,dateString,url);
